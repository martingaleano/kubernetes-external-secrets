...
describe('SystemManagerBackend', () => {
  ...
  describe('_get', () => {
    ...
    let getParameterHistoryPromise

    beforeEach(() => {
      getParameterPromise = sinon.mock()
      getParameterPromise.promise = sinon.stub()
      getParameterHistoryPromise = sinon.mock()
      getParameterHistoryPromise.promise = sinon.stub()
      clientMock.getParameter = sinon.stub().returns(getParameterPromise)
      clientMock.getParameterHistory = sinon.stub().returns(getParameterHistoryPromise)
    })
    ...
    it('returns secret property value', async () => {
      ...
      getParameterHistoryPromise.promise.resolves({
        Parameters: [{
          Value: 'fakeValue',
          LastModifiedDate: 1596037422.449
        }]
      })
    ...
    it('throws a meaningful message when the parameter does not exist', async () => {
      ...
      getParameterHistoryPromise.promise.rejects(error)
