...
class SystemManagerBackend extends KVBackend {
   ...
   */
  constructor ({ clientFactory, assumeRole, logger, checkUpdated, intervalPolling }) {
    super({ logger })
    this._client = clientFactory()
    this._clientFactory = clientFactory
    this._assumeRole = assumeRole
    this._checkUpdated = checkUpdated
    this._intervalPolling = intervalPolling
  }
  ...
  async _get ({ key, specOptions: { roleArn, region } }) {
    ...
    try {
      ...
      const check = await client
        .getParameterHistory(params)
        .promise()

      while (check.NextToken) {
        params.NextToken = check.NextToken
        await client.getParameterHistory(params).promise()
      }
      const timetocompare = new Date(Date.now() - this._intervalPolling)

      if (check.Parameters[check.Parameters.length - 1].LastModifiedDate > timetocompare || !this._checkUpdated) {
        ...
        throw Object.assign(
          new Error('The parameter has not changed since last time'),
          { code: 'SecretNotChanged' }
        )
      }
    } catch (err) {
