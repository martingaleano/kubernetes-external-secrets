...
class Poller {
  ...
  async _poll () {
    ...
    try {
      ...
      if (err.code === 'ParameterNotChanged') {
        this._logger.error(err, ` the secret ${this._namespace}/${this._name} has not changed`)
        await this._updateStatus(`${err.message}`)
      } else {
  ...
  async _upsertKubernetesSecret () {
    ...
    this._logger.info(secretManifest)

    try {
      if (secretManifest !== null) return await kubeNamespace.secrets.post({ body: secretManifest })
    } catch (err) {
